...
public class TermTest {
    ...
    public void testGetSetCoefficient() {
        ...
        for(int c : COEFFICIENTS) {
            ...
            assertEquals(c, test.getCoefficient(), "Expected and actual coefficients DON'T match");
        }
    }
    ...
    public void testGetSetExponent() {
        ...
        for(int e : EXPONENTS) {
            test.setExponent(e);
            assertEquals(e, test.getExponent(), "Expected and actual exponents DON'T match");
        }
    }
...
//            assertEquals("Expected and actual toStrings DON'T match", expected, actual);
    ...
    public void testDefaultConstructor() {
        ...
        assertEquals(1, test.getCoefficient(), "Expected and actual coefficients DON'T match");
        assertEquals(1, test.getExponent(), "Expected and actual exponents DON'T match");
    }
    ...
    public void testCopyConstructor() {
        ...
        assertEquals(
                original.getCoefficient(), copy.getCoefficient(),"Coefficients in 'copy' does not match 'original'");
        assertEquals(
                original.getExponent(), copy.getExponent(),"Exponents in 'copy' does not match 'original'");
        assertTrue(
                original != copy,"Reference is not a unique memory address (shallow copy)");
    }
    ...
    public void testEqualsTrue() {
        ...
        for(int i = 0; i < COEFFICIENTS.length; i++) {
            ...
            assertEquals(test1, test2,"Expected and actual should be TRUE for equals");
        }
    }
    ...
    public void testEqualsFalse() {
        ...
        for(int i = 1; i < COEFFICIENTS.length; i++) {
            ...
            assertNotEquals(test1, test2,"Expected and actual should be FALSE for equals");
        }
    }
    ...
    public void testCompareTo() {
        ...
        for(int i = 0; i < EXPONENTS.length; i++) {
            ...
            if( COMPARE_TO_VALUES[i] == ORIGINAL_EQUALS_TEST) {
                assertEquals(
                        0, compareResult,"Expected and actual compareTo result (original exponent EQUALS test exponent) DON'T match");
            }
            else if ( COMPARE_TO_VALUES[i] == ORIGINAL_BEFORE_TEST) {
                assertTrue(
                        compareResult < 0,"Expected and actual compareTo result (original exponent COMES BEFORE test exponent) DON'T match");
            } else {
                assertTrue(
                        compareResult > 0,"Expected and actual compareTo result (original exponent COMES AFTER test exponent) DON'T match");
            }
